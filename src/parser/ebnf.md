...
\textit{ColDef} \quad &::= \quad \textit{Id} \ \textit{Type} \ [ \text{ PRIMARY KEY } ] \ [ \text{ INDEX } \ \textit{IndexTech} ] \\
\textit{IndexTech} \quad &::= \quad \text{SEQUENTIAL} \ | \ \text{HASH} \ | \ \text{BTREE} \ | \ \text{RTREE} \\
\\
\textit{SelectStmt} \quad &::= \quad \text{SELECT } \ \textit{Cols} \ \text{ FROM } \ \textit{Id} \ [ \text{ WHERE } \ \textit{Condition} ] \ [ \text{ ORDER BY } \ \textit{Id} ] \\
\textit{Cols} \quad &::= \quad \text{*} \ | \ \textit{Id} \ \{ \text{ , } \ \textit{Id} \}^* \\
\textit{Condition} \quad &::= \quad \textit{Id} \ \textit{RelOp} \ \textit{Value} \\
& | \quad \textit{Id} \ \text{ BETWEEN } \ \textit{Value} \ \text{ AND } \ \textit{Value} \\
& | \quad \textit{Id} \ \text{ IN } \ \text{(} \ \textit{SpatialCond} \ \text{)} \\
\textit{SpatialCond} \quad &::= \quad \text{POINT} \ \text{(} \ \textit{SignedNum} \ \text{,} \ \textit{SignedNum} \ \text{)} \ \text{,} \ ( \text{RADIUS } \ \textit{Number} \ | \ \text{K } \ \textit{Number} ) \\
\\
...
\textit{Type} \quad &::= \quad \text{INT} \ | \ \text{FLOAT} \ | \ \text{VARCHAR} \ [ \text{(} \ \textit{Number} \ \text{)} ] \ | \ \text{POINT} \\
\textit{Value} \quad &::= \quad [ \text{-} ] \ \textit{Number} \ | \ \textit{String} \\
\textit{SignedNum} \quad &::= \quad [ \text{-} ] \ \textit{Number} \\
\end{aligned}
$$
...
```sql
CREATE TABLE Estudiantes (id INT PRIMARY KEY, nombre VARCHAR(30), nota FLOAT);
...
SELECT * FROM Estudiantes ORDER BY nota;
